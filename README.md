...
`value?.let(block)` and `value.let?.(block)` use null-safe helpers — the block is skipped when the receiver is `null`/`undefined`. A chain like `a?.b.let(block)` does _not_: the short-circuit lives inside the receiver, so the plain `let` helper is used (matching Kotlin).
