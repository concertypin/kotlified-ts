...
const INLINE_RUNTIME: Record<string, string> = {
    ...
    __kt$takeIfOrNull:
        "(v, p) => (v == null ? undefined : (p(v) ? v : undefined))",
    __kt$takeUnless: "(v, p) => (p(v) ? undefined : v)",
    __kt$takeUnlessOrNull:
        "(v, p) => (v == null ? undefined : (p(v) ? undefined : v))",
};

/** Fast path: only bother parsing when a candidate call might exist. */
const CANDIDATE_RE =
    /\.\s*(?:let|apply|run|also|takeIf|takeUnless)\s*(?:\?\.)?\s*\(/;
...
function isMemberLike(
    n: any
): n is t.MemberExpression | t.OptionalMemberExpression {
    return (
...
function isOptionalAccess(
    ...
    return (
        callee?.type === "OptionalMemberExpression" && callee.optional === true
    );
}
...
const SHADOW_PROPERTY_TYPES = new Set([
    "ObjectProperty",
    "ClassProperty",
...
function findShadowedNames(ast: t.File): Set<string> {
    ...
    walk(ast, (node) => {
        if (SHADOW_METHOD_TYPES.has(node.type)) {
            const key = node.key;
            if (t.isIdentifier(key) && METHOD_NAMES.has(key.name))
                names.add(key.name);
            return;
        }
...
function findCandidates(ast: t.File): Candidate[] {
    ...
    walk(ast, (node) => {
        ...
        const helper = isOptionalAccess(node)
            ? HELPERS[name]!.orNull
            : HELPERS[name]!.base;
        candidates.push({
            node,
...
export function transformCode(
    ...
    try {
        ...
        console.warn(
            `[kotlify] skipped ${options.filename ?? "<unknown>"}: ${message}`
        );
        return null;
    }
    ...
    const roots = candidates.filter(
        (c) =>
            !candidates.some(
                (o) => o !== c && o.start <= c.start && c.end <= o.end
            )
    );
    ...
    if (helpers.length > 0) {
        if (ast.program.sourceType === "module") {
            ...
            const specifiers = helpers
                .map((h) => `${RUNTIME_EXPORT[h]} as ${h}`)
                .join(", ");
            out = insertAtTop(
                out,
                `import { ${specifiers} } from ${JSON.stringify(runtimeId)};\n`
            );
        } else {
            const definitions = helpers
                .map((h) => `const ${h} = ${INLINE_RUNTIME[h]};`)
                .join("\n");
    ...
    if (options.shadowWarn !== false && shadowed.size > 0) {
        ...
        if (risky.length > 0) {
            const names = [...shadowed]
                ...
                .sort()
                .join(", ");
            const lines = [...new Set(risky.map((c) => c.line))]
                .sort((a, b) => a - b)
                .join(", ");
