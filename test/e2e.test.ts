...
import kotlify from "../src/index.ts";
...
`
            ...
            const result = await build({
                ...
                logLevel: "silent",
                plugins: [
                    kotlify({ runtimeId: path.join(root, "src/runtime.ts") }),
                ],
                build: {
                    write: false,
                    minify: false,
            ...
            const chunks = (
                result as unknown as { output: Array<{ code: string }> }
            ).output;
...
`
            ...
            execFileSync(
                process.execPath,
                [tsc, "-p", path.join(dir, "tsconfig.json")],
                {
                    stdio: "pipe",
                }
            );
