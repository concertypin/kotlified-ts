...
import { expect } from "./_expect.ts";
import { shouldTransformId } from "../src/index.ts";
...
describe("shouldTransformId", () => {
  ...
  it("transforms SFC script blocks", () => {
    expect(shouldTransformId("/src/App.vue?vue&type=script&setup=true&lang.ts")).toBe(true);
    expect(shouldTransformId("/src/App.svelte?svelte&type=script&lang=ts")).toBe(true);
  });
